...
node *merge_sort(node* head,int start,int end)
{
    ...
    node* midtemp=findAtPositionK(head,mid);
    node* head2=midtemp->next;
    midtemp->next=NULL;
    
    int start1=start;
    int end1=mid;
    int start2=end-(mid+1);
    int end2=end;
    
    merge_sort(head,start1,end1);
    merge_sort(head2,start2,start2);
    mergeLL(head,head2);
